...
from app.database import save_user, get_all_users, get_user
import glv
...
async def notify_users_about_expiry():
    while True:
        ...
        for user in users:
            try:
                vpn_user = await find_user_by_username(user['username'])
                if not vpn_user:
                    continue

                expire_timestamp = vpn_user.get("expire")
                if not expire_timestamp:
                    continue

                time_left = expire_timestamp - now

                if 0 < time_left < 3600 * 24 + 60 and not user.get('notified_1_day'):  # За день
                    await bot.send_message(user['telegram_id'], "⚡️ Ваша подписка истекает через 24 часа! Продлите, чтобы избежать отключения.")
                    mark_user_notified(user['telegram_id'], 'notified_1_day')

                if 0 < time_left < 3600 * 3 + 60 and not user.get('notified_3_hours'):  # За 3 часа
                    await bot.send_message(user['telegram_id'], "🚨 Осталось меньше 3 часов до окончания подписки! Поторопитесь с оплатой!")
                    mark_user_notified(user['telegram_id'], 'notified_3_hours')
